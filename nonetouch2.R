...
vel[,1] <- pos[,2]*.02*(r^(-2.5))  
vel[,2] <- -pos[,1]*.02*(r^(-2.5))  
vel[1,] <- c(0,0)
pos[1,] <- c(0,0)
acc <- matrix(rep(0,2*N), ncol=2)
mass <- pmax(c(50,20*.4^(2:(N))),.01)
col <- hsv(runif(N),runif(N,0,.6),1, alpha = .03)
png(sprintf("pngs/nbody%05d.png",l), width=3000, height=3000, type="cairo")
par(bg="black")
...
for(i in 1:100000){
  ...
  if(i>0000) points(pos[,1]-pos[1,1],pos[,2]-pos[1,2], col=col, pch=20, cex=4*mass^0.5)
}
text(-4,-4.9, labels = paste0("Seed = ",seed), cex=4, col="white")
dev.off()
#shell("nbody.png")
}