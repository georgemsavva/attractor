...
for(i in 101) {
  ...
  for(j in 1:N){
    ...
    george1 <- imageAttractorCPP(startPos = c(0.5,0.5), 
                                 ...
                                 mutation=mutation)
    cimg1 <- makeImg(.997^(george1), ramp)
    save.image(cimg1, file=sprintf("pngs/zzzpink_%d.png",j))
  }
}

shell("ffmpeg -r 30 -y -i pngs/zzzpink_%d.png -c:v libx264 -r 30 -pix_fmt yuv420p yellow.mp4")
shell("yellow.mp4")
